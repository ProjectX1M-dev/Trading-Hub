...
class MT5ApiService {
  ...
  constructor() {
    ...
    this.baseURL = import.meta.env.VITE_MT5_API_URL || 'https://mt5.mtapi.io';
    ...
    this.apiClient.interceptors.response.use(
      ...
      (error) => {
        if (error.code === 'ECONNREFUSED') {
          throw new Error('Cannot connect to MT5 API server. Please ensure the API server is running and accessible.');
        }
        ...
        if (error.response?.status >= 500) {
          throw new Error('MT5 API server error. Please try again later.');
        }
        throw error;
      }
    );
